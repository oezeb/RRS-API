...
def init_schema(cursor):
    def execute(sql):
        ...
        for res in it:
            click.echo(f"Running query: {res}")

    click.echo("Creating tables...")
    with current_app.open_resource('db/schema.sql') as f:
        execute(f.read())

    click.echo("Populating tables with initial data...")
    init_data(cursor)

    click.echo("Creating triggers...")
    variables = globals()
...
def init_data(cursor):
    sql = f"INSERT INTO {ResvPrivacy.TABLE} (privacy, label) VALUES (%s, %s)"
    ...
    sql = f"INSERT INTO {ResvStatus.TABLE} (status, label) VALUES (%s, %s)"
    cursor.executemany(sql, [
    ...
    sql = f"INSERT INTO {RoomStatus.TABLE} (status, label) VALUES (%s, %s)"
    cursor.executemany(sql, [
        (RoomStatus.UNAVAILABLE, '不可用'),
        (RoomStatus.AVAILABLE, '可用')
    ])

    sql = f"INSERT INTO {UserRole.TABLE} (role, label) VALUES (%s, %s)"
    cursor.executemany(sql, [
...
# Sessions Triggers
# ==============================================================
# TODO: When updating session time, make sure that the
# time slots for all reservations that are related to the session
# are still within the session time range.
